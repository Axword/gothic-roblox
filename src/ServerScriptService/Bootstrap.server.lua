...
local Constants=require(ReplicatedStorage.Shared.Constants);local DataIndex=require(ReplicatedStorage.Shared.DataIndex);local State=require(script.Services.StateService);local Save=require(script.Services.SaveService);local Combat=require(script.Services.CombatService);local Inventory=require(script.Services.InventoryService);local Trainers=require(script.Services.TrainerService);local Dialogues=require(script.Services.DialogueService);local World=require(script.Services.WorldService)
...
action.OnServerEvent:Connect(function(player,kind:string,payload:any)
 ...
 elseif kind=="inventoryActivate" and type(payload)=="string" then local _,message=Inventory.activate(player,payload);notice:FireClient(player,"toast",message)
