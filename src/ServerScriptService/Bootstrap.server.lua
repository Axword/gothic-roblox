...
action.OnServerEvent:Connect(function(player,kind:string,payload:any)
 ...
 elseif kind=="lockStart" and payload=="chest_tutorial" then if State.get(player).openedChests.chest_tutorial then notice:FireClient(player,"toast","Skrzynia jest pusta.") else locks[player]=1;notice:FireClient(player,"lockStep","left") end
