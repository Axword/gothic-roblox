...
for _, name in {"quests_main","quests_old_faction","quests_new_faction","quests_side"} do for id,q in DataIndex.byId(name) do quests[id]=q end end
...
function QuestService.chooseFaction(player:Player,faction:string):boolean
 ...
 local count=0;for id,status in s.quests do if status=="complete" and string.find(id,"quest_"..(faction=="kordon" and "old" or "new")) then count+=1 end end
